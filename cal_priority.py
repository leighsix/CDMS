...
from cal_map_view import PriorityCalMapView
...
class CalPriorityWindow(QDialog):
    """저장된 자산을 보여주는 창"""
    ...
    def update_map(self):
        ...
        if selected_assets:
            PriorityCalMapView(selected_assets, self.map)
        data = io.BytesIO()
    ...
    def get_selected_assets(self):
        ...
        for row in range(self.assets_table.rowCount()):
            ...
            if checkbox_widget and checkbox_widget.isChecked():
                ...
                selected_assets.append((priority, unit, asset_name, area, coordinate, engagement_effectiveness, bmd_priority))
        return selected_assets

    def load_assets(self):
        """데이터베이스에서 자산 정보를 로드하여 테이블에 표시하는 함수"""
        # 기존의 cal_assets_priority_ko 테이블에서 우선순위 정보 확인
        self.parent.cursor.execute("SELECT COUNT(*) FROM cal_assets_priority_ko")
    ...
    def update_table(self, df):
        ...
        # 열 숨기기
        hidden_columns = [2, 4, 5, 6, 9, 10, 11, 12, 13, 14, 15, 18, 19, 20, 21]
...
class MainWindow(QMainWindow, QObject):
    ...
    def __init__(self):
        ...
        self.view_assets_page = CalPriorityWindow(self)
